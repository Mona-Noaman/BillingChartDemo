...
public class VFGAnimatedSplashViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        _ = Timer.scheduledTimer(timeInterval: TimeInterval(delayAmount),
                             target: self,
    ...
    override public func present(_ viewControllerToPresent: UIViewController,
                                 ...
                                 completion: (() -> Void)? = nil) {
        if shouldPresent {
            animationStart(viewControllerToPresent,
                           animated: flag,
    ...
    func animationStart(_ viewControllerToPresent: UIViewController,
                        animated flag: Bool,
                        completion: (() -> Void)? = nil) {
        start(nextViewController: viewControllerToPresent)
        DispatchQueue.main.asyncAfter(deadline: .now() + presentDelay, execute: {[weak self] in
    ...
    @objc func executeAnimation() {
        ...
        animationStart(nextViewController,
                       animated: flag,
                       completion: completion)
    }

    func presentOnSuper(_ viewControllerToPresent: UIViewController,
                        animated flag: Bool,
                        completion: (() -> Void)? = nil) {
    ...
    func splashToDashboardLogoAnimations(animated: Bool = true, completion: (() -> Void)? = nil) {
        guard let splashDelegate = nextViewController as? VFGMVa10SplashDelegate else {
            return
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + (animated ? 1.1 : 0)) { [weak self] in
            ...
            UIView.transition(with: self.splashLogo,
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + (animated ? 1.1 : 0)) { [weak self] in
            ...
            UIView.transition(with: self.backgroundImageView,
                              ...
                              options: .transitionCrossDissolve,
                              animations: {
                                self.backgroundImageView.image = splashDelegate.nextViewControllerBGImage
    ...
    func splashToGenericLogoAnimations(completion: (() -> Void)? = nil) {
        UIView.animateKeyframes(withDuration: SplashToGenericAnimationConstants.duration,
                                delay: SplashToGenericAnimationConstants.delay, options: .calculationModeLinear,
                                animations: {[weak self] in
                                    //logo scale down
                                    UIView.addKeyframe(
                                        withRelativeStartTime: SplashToGenericAnimationConstants.scaleDownStartTime,
                                        relativeDuration:
                                        SplashToGenericAnimationConstants.scaleDownDuration,
                                        animations: {[weak self] in
                                            guard let `self` = self else {
                                                return
                                            }
